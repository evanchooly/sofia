...
public class LocalizerGenerator {
	...
	public LocalizerGenerator(SofiaConfig sofiaConfig) throws IOException {
		...
		Map<String, List<Method>> loggers = new HashMap<String, List<Method>>();
			...
			if (method.getLogged()) {
				...
				if (level == null) {
					throw new IllegalArgumentException(String.format(
							"Invalid logging level '%s' for logging type '%s'",
							method.getLogLevel(), config.getType()));
				}
				level.add(method);
			}
		}
	...
	public void write() {
		File file = new File(outputDirectory, String.format("%s/%s.java",
				pkgName.replace('.', '/'), capitalize(bundleName)));
		file.getParentFile().mkdirs();
		try {
			PrintWriter stream = new PrintWriter(file, "UTF-8");
			stream.println(this);
			stream.flush();
		} catch (Exception e) {
			throw new RuntimeException(e.getMessage(), e);
